...
/**
 * One card per stat. This used to be inlined in the .map() below, which called
 * useCountUp inside a callback — a rules-of-hooks violation that only happened
 * to work because `stats` is a fixed-length constant.
 */
function StatCard({ item, index, visible }) {
  ...
  return (
    <div className={`flex items-center justify-center gap-4 px-4 py-7 sm:px-6 sm:py-8 ${borderClasses}`}>
    ...
    <section ref={sectionRef} className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12">
      <div className="bg-[#051124] text-white rounded-md shadow-2xl border border-gray-800/60 grid grid-cols-2 lg:grid-cols-4">
        {stats.map((item, index) => (
          <StatCard key={item.id} item={item} index={index} visible={visible} />
        ))}
      </div>
    </section>
  );
